...
factor sampler. Factor sampling and relation encoding are performed once and
amortized across the ten queries in a block.
"""
...
def create_aligned_single_subsets(conn, source: str, truth: str, prefix: str,
    ...
    def make_full(source_table: str, target_table: str,
                  selected: Sequence[Tuple[str, str]]) -> None:
        projection = ", ".join(
            "s.%s AS %s" % (qident(actual), qident(normalized))
            for actual, normalized in selected
        )
        cursor.execute("DROP TABLE IF EXISTS %s" % qident(target_table))
        cursor.execute(
            "CREATE UNLOGGED TABLE %s AS SELECT "
            "row_number() OVER (ORDER BY s.ctid)::bigint AS %s, %s "
            "FROM %s s" % (
                qident(target_table), qident("_rid"), projection,
                qident(source_table),
            )
        )
        cursor.execute(
            "CREATE UNIQUE INDEX ON %s (%s)" % (
                qident(target_table), qident("_rid"),
            )
        )
        cursor.execute("ANALYZE %s" % qident(target_table))

    if row_limit <= 0:
        make_full(source, pred_table, source_cols)
        make_full(truth, truth_table, truth_cols)
        conn.commit()
...
def create_aligned_join_subsets(conn, pred_left: str, pred_right: str,
    ...
    cursor.execute("DROP TABLE IF EXISTS %s" % qident(key_table))
    table_kind = "UNLOGGED" if key_limit <= 0 else "TEMP"
    on_commit = "" if key_limit <= 0 else " ON COMMIT PRESERVE ROWS"
    common_sql = (
        "CREATE %s TABLE %s%s AS "
        ...
        % (
            ...
            qident(key_table),
            on_commit,
            qident(pred_left_key),
            qident(pred_left),
            qident(pred_right),
            qident(pred_left_key),
            qident(pred_right_key),
            qident(truth_left),
            qident(truth_left_key),
            qident(pred_left_key),
            qident(truth_right),
            qident(truth_right_key),
            qident(pred_left_key),
            qident(pred_left_key),
            qident(pred_left_key),
        )
    ...
    if key_limit <= 0:
        ...
        cursor.execute(
            ...
            % (qident(pred_left_key), int(seed), int(key_limit))
        )
    ...
    def make(source_table: str, target_table: str) -> None:
        ...
        cursor.execute(
            ...
            "FROM %s s JOIN %s k ON s.%s=k.join_key" % (
                table_kind, qident(target_table), on_commit,
                qident("_rid"), projection, qident(source_table),
...
def query_attributes(query: str) -> Tuple[str, ...]:
...
def required_missing_attributes(
    ...
    table_columns = [
        set(columns(engine.connection, table))
        for table in prepared["pred_tables"]
    ]
    required = [set(), set(), set()]
    for query in meta["queries"]:
        spec = parse_query(query)
        attributes = query_attributes(query)
        if spec.right_token:
            for attribute in attributes:
                if attribute == spec.join_column:
                    required[1].add(attribute)
                    required[2].add(attribute)
                    continue
                if attribute in table_columns[1]:
                    required[1].add(attribute)
                if attribute in table_columns[2]:
                    required[2].add(attribute)
        else:
            required[0].update(attributes)

    if workload == "set" and meta.get("semantic_relations"):
        required[0].update(required[1])
        required[0].update(required[2])

    result: List[Tuple[str, ...]] = []
    for position, table in enumerate(prepared["pred_tables"]):
        actual_missing = set(engine.missing_attributes(table))
        _configured, ordering = relation_settings(
            meta, position, workload, factor_map
        )
        active = set(
            attribute for attribute in required[position]
            if attribute in actual_missing
        )
        pending = list(active)
        while pending:
            attribute = pending.pop()
            for dependency in ordering.get(attribute, tuple()):
                if dependency in actual_missing and dependency not in active:
...
def create_deduplicated_right(conn, left: str, right: str, key: str, name: str) -> str:
    ...
    cursor.execute(
        "CREATE UNLOGGED TABLE %s AS SELECT %s FROM %s r" % (
...
def project_joint_bundle(conn, bundle, source_table: str, join_key: str,
    ...
    for attribute in relation_columns:
        ...
        if attribute in bundle.sample_tables:
            selected.append(bundle.sample_column(attribute))
        if attribute in bundle.inline_distributions:
            selected.append(bundle.donor_column(attribute))
    ...
    kept_inline = tuple(
        attribute for attribute in bundle.inline_distributions
        if attribute in relation_columns
    )
    return dataclasses.replace(
        ...
        inline_distributions=kept_inline,
        sampling_s=0.0,
        encoding_s=0.0,
    )
...
def factor_grouping_query(
    relation: str,
    separators: Sequence[str],
    masks: Optional[Sequence[int]],
    where: str,
    aggregates: str,
    value: Optional[str] = None,
    use_union: bool = True,
    ...
    if not separators:
        ...
        grouped = []
        if value is not None:
            selected.append("s.%s AS value" % qident(value))
            grouped.append("s.%s" % qident(value))
        selected.extend(["0::bigint AS context_hash", "0 AS gmask", aggregates])
        group_clause = " GROUP BY " + ", ".join(grouped) if grouped else ""
        return "SELECT %s FROM %s s WHERE %s%s" % (
            ", ".join(selected), relation, where, group_clause
        )

    normalized_masks = tuple(
        int(mask) for mask in (
            masks if masks is not None else range(1 << len(separators))
        )
    )
    if not use_union:
        selected = ["s.%s" % qident(column) for column in separators]
        if value is not None:
            selected.append("s.%s AS value" % qident(value))
        selected.extend(
            [
                "%s AS context_hash" % grouping_context_hash("s", separators),
                "GROUPING(%s) AS gmask"
                % ", ".join(qident(column) for column in separators),
                aggregates,
            ]
        )
        grouped = grouping_sets(separators, value, normalized_masks)
        return "SELECT %s FROM %s s WHERE %s GROUP BY GROUPING SETS (%s)" % (
            ", ".join(selected), relation, where, grouped
        )

    def branch(mask: int, false_branch: bool = False) -> str:
        included = [
            column
            for index, column in enumerate(separators)
            if not (mask & (1 << (len(separators) - index - 1)))
        ]
        included_set = set(included)
        selected = [
            "s.%s" % qident(column)
            if column in included_set
            else "NULL AS %s" % qident(column)
            for column in separators
        ]
        context_values = [
            "s.%s" % qident(column) if column in included_set else "NULL"
            for column in separators
        ]
        grouped = ["s.%s" % qident(column) for column in included]
        if value is not None:
            selected.append("s.%s AS value" % qident(value))
            grouped.append("s.%s" % qident(value))
        selected.extend(
            [
                "hashtextextended(ROW(%s)::text,0) AS context_hash"
                % ", ".join(context_values),
                "%d AS gmask" % mask,
                aggregates,
            ]
        )
        branch_where = "FALSE" if false_branch else where
        group_clause = " GROUP BY " + ", ".join(grouped) if grouped else ""
        return "SELECT %s FROM %s s WHERE %s%s" % (
            ", ".join(selected), relation, branch_where, group_clause
        )

    branches = []
    if 0 not in normalized_masks:
        branches.append(branch(0, True))
    branches.extend(branch(mask) for mask in normalized_masks)
...
def grouping_context_hash(alias: str, separators: Sequence[str]) -> str:
    if not separators:
        return "0::bigint"
    values = ", ".join("%s.%s" % (alias, qident(column)) for column in separators)
    return "hashtextextended(ROW(%s)::text,0)" % values
...
def grouped_match(
    ...
    base_values = tuple(base_columns or separators)
    if len(base_values) != len(separators):
        raise ValueError("Factor and base contexts must have the same arity")
    terms = [
        "%s.gmask=(%s)" % (stats_alias, grouping_mask(base_alias, base_values)),
        "%s.context_hash=(%s)" % (
            stats_alias, grouping_context_hash(base_alias, base_values)
        ),
    ]
    terms.extend(
        "%s.%s IS NOT DISTINCT FROM %s.%s" % (
            stats_alias, qident(column), base_alias, qident(base_column)
        )
        for column, base_column in zip(separators, base_values)
    )
...
def aggregate_sql(query: str, relation: str, orderings: Mapping[str, Sequence[str]],
    ...
    summarize_base = (
        summarize_direct and direct
        and y not in by_attr and y not in group_cols
    )
    base_context_specs: List[Tuple[str, Tuple[str, ...], Tuple[str, ...]]] = []
    ...
    def base_context(
        ...
        if not summarize_base or not values:
            return values
        aliases = tuple(
            "__cade_context_%s_%d_%d" % (kind, index, position)
            for position in range(len(values))
        )
        base_context_specs.append((target, values, aliases))
        return aliases

    for index, (attr, predicates) in enumerate(by_attr.items()):
        if summarize_base:
            predicate_flag = qident("__cade_predicate_%d" % index)
            observed_expr = "b.%s" % predicate_flag
            missing_expr = "b.%s IS NULL" % predicate_flag
        else:
            observed_expr = " AND ".join(
                predicate_sql("b", predicate) for predicate in predicates
            )
            missing_expr = "b.%s IS NULL" % qident(attr)
        ...
        ctes.append(
            "%s AS MATERIALIZED (%s)" % (
                alias,
                factor_grouping_query(
                    working_relation,
                    separators,
                    (grouping_patterns or {}).get(tuple(separators)),
                    where,
                    "SUM(CASE WHEN %s THEN 1.0 ELSE 0.0 END)/"
                    "NULLIF(COUNT(*)::double precision,0) AS p" % stat_predicate,
                    use_union=direct,
                ),
            )
        )
        lateral = "lp%d" % index
        base_separators = base_context("predicate", index, attr, separators)
        joins.append(
            "LEFT JOIN %s %s ON %s" % (
                alias,
                lateral,
                grouped_match("b", lateral, separators, base_separators),
            )
        )
        weight_terms.append(
            "CASE WHEN %s THEN COALESCE(%s.p,0.0) ELSE CASE WHEN %s THEN 1.0 ELSE 0.0 END END" % (
                missing_expr, lateral, observed_expr
            )
        )
    ...
    for index, attr in enumerate(group_cols):
        ...
        ctes.append(
            ...
            "%s AS MATERIALIZED (SELECT *, SUM(n) OVER (PARTITION BY gmask%s) AS total FROM %s_raw)" % (
                ...
                alias,
                (", " + ", ".join(qident(value) for value in separators))
                if separators else "",
                alias,
            )
        )
        lateral = "lg%d" % index
        base_separators = base_context("group", index, attr, separators)
        joins.append(
    ...
    if direct and y not in missing:
        ...
        ctes.append(
            "qy AS MATERIALIZED (%s)" % factor_grouping_query(
                working_relation,
                y_separators,
                (grouping_patterns or {}).get(tuple(y_separators)),
                y_where,
                "SUM(s.%s::double precision) AS sy, "
                "COUNT(*)::double precision AS n" % qident(y),
            )
        )
        base_separators = base_context("aggregate", 0, y, y_separators)
        joins.append(
            "LEFT JOIN qy ly ON %s" % grouped_match(
                "b", "ly", y_separators, base_separators
            )
        )
    ...
    if summarize_base:
        ...
        base_select_items: List[str] = []
        base_group_items: List[str] = []
        for column in group_cols:
            if column == y or column in base_columns:
                continue
            expression = "s.%s" % qident(column)
            base_columns.append(column)
            base_select_items.append(expression)
            base_group_items.append(expression)
        for index, (attribute, predicates) in enumerate(by_attr.items()):
            predicate_expression = " AND ".join(
                predicate_sql("s", predicate) for predicate in predicates
            )
            flag_expression = (
                "CASE WHEN s.%s IS NULL THEN NULL "
                "WHEN %s THEN TRUE ELSE FALSE END" % (
                    qident(attribute), predicate_expression,
                )
            )
            base_select_items.append(
                "%s AS %s" % (
                    flag_expression,
                    qident("__cade_predicate_%d" % index),
                )
            )
            base_group_items.append(flag_expression)
        for target, separators, aliases in base_context_specs:
            for column, alias in zip(separators, aliases):
                expression = "CASE WHEN s.%s IS NULL THEN s.%s ELSE NULL END" % (
                    qident(target),
                    qident(column),
                )
                base_select_items.append("%s AS %s" % (expression, qident(alias)))
                base_group_items.append(expression)
        base_select = ", ".join(base_select_items)
        base_prefix = base_select + ", " if base_select else ""
        base_group = (
            " GROUP BY " + ", ".join(base_group_items)
            if base_group_items else ""
        )
        ctes.append(
...
def assign_marked_null_symbols(
    conn,
    tables: Sequence[str],
    group_size: int,
    seed: int,
) -> Dict[str, int]:
    """Assign one marked-null symbol to each group of missing cells.

    This applies the grouping rule from ``inject_marked_nulls.py`` to the
    prepared relations. It changes no attribute value and introduces no
    additional missing cells. The symbols therefore annotate the same input
    relations used by the ordinary-null experiment.
    """
    if group_size < 2:
        raise ValueError("The marked-null group size must be at least two")
    ...
    for table_position, table in enumerate(tables):
        ...
        for attribute_position, attribute in enumerate(engine.missing_attributes(table)):
            symbol_column = "%s_nullsym" % attribute
            if symbol_column not in present:
                cursor.execute(
                    "ALTER TABLE %s ADD COLUMN %s text" % (
                        qident(table), qident(symbol_column),
                    )
                )
                symbol_prefix = "marked_%s_%s_" % (
                    clean_name(table)[:24], clean_name(attribute)[:18],
                )
                symbol_seed = (
                    int(seed)
                    + (table_position + 1) * 1000003
                    + (attribute_position + 1) * 1009
                )
                cursor.execute(
                    "WITH ranked AS ("
                    "SELECT %s, row_number() OVER ("
                    "ORDER BY hashtextextended(%s::text || %s, %s), %s"
                    ") AS marked_rank FROM %s WHERE %s IS NULL"
                    ") UPDATE %s target SET %s = %s || "
                    "((ranked.marked_rank - 1) / %s)::text "
                    "FROM ranked WHERE target.%s = ranked.%s" % (
                        qident("_rid"),
                        qident("_rid"),
                        literal(":" + attribute),
                        int(symbol_seed),
                ...
                present.add(symbol_column)
            cursor.execute(
                "WITH symbol_sizes AS ("
                "SELECT %s AS symbol, count(*)::integer AS symbol_size "
                "FROM %s WHERE %s IS NULL GROUP BY %s"
                ") SELECT "
                "(SELECT count(*) FROM %s WHERE %s IS NULL), "
                "(SELECT count(*) FROM symbol_sizes), "
                "(SELECT count(*) FROM symbol_sizes WHERE symbol_size > 1), "
                "(SELECT count(*) FROM %s WHERE "
                "(%s IS NULL) <> (%s IS NOT NULL)), "
                "COALESCE((SELECT max(symbol_size) FROM symbol_sizes), 0)" % (
                    qident(symbol_column),
                    qident(table),
                    qident(attribute),
                    qident(symbol_column),
            ...
            null_count, symbol_count, repeated_count, invalid_count, largest_group = cursor.fetchone()
            if invalid_count:
                raise ValueError(
                    "%s.%s has %d cells with inconsistent marked-null symbols" %
                    (table, attribute, invalid_count)
                )
            if largest_group > group_size:
                raise ValueError(
                    "%s.%s has a marked-null group of size %d" %
                    (table, attribute, largest_group)
                )
            total_nulls += int(null_count)
            total_symbols += int(symbol_count)
            repeated_symbols += int(repeated_count)
        cursor.execute("ANALYZE %s" % qident(table))
    conn.commit()
    return {
        "marked_null_count": total_nulls,
        "marked_symbol_count": total_symbols,
        "marked_repeated_symbol_count": repeated_symbols,
    }


def build_camc(conn, meta: Mapping[str, Any], prepared: Mapping[str, Any],
               workload: str, factor_map: Mapping[str, Sequence[str]], h: int,
               seed: int, strict: bool = True) -> Tuple[NativeMCDB, Dict[str, Any], float, float, int]:
    engine = NativeMCDB(conn)
    ...
    fallback_symbols = 0
    if workload == "set" and meta.get("semantic_relations"):
        _configured_missing, ordering = relation_settings(
            meta, 0, workload, factor_map
        )
        missing = required_by_position[0]
    ...
    for position, table in enumerate(prepared["pred_tables"]):
        ...
        ordering = {attr: tuple(value for value in separators if value in available)
                    for attr, separators in ordering.items() if attr in available}
        missing = required_by_position[position]
        for attr in missing:
            ordering.setdefault(attr, tuple())
        bundle = engine.create_bundle(
            table, missing, ordering, h, seed=float(seed % 1000) / 1000.0,
            prefix="camc_%s_%d" % (clean_name(table), h), strict=strict,
            n_bins=5,
...
def run_set_block(conn, dataset: str, block: str, meta: Mapping[str, Any],
                  ...
                  sampling_share: float, encoding_share: float,
                  camc_engine: NativeMCDB,
                  camc_mappings: Mapping[str, Any], query_limit: Optional[int] = None,
                  completed_queries: Optional[Iterable[int]] = None,
                  checkpoint=None,
                  methods: Sequence[str] = METHODS) -> List[Dict[str, Any]]:
...
def run_agg_block(conn, dataset: str, block: str, meta: Mapping[str, Any],
                  ...
                  h: int, timeout_s: int, sampling_share: float,
                  encoding_share: float,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Comma-separated subset of CAEX,CAMC,CADE",
    )
    ...
    parser.add_argument(
        "--marked-null-group-size",
        type=int,
        default=0,
        help="Group missing cells under shared marked-null symbols; zero keeps ordinary nulls",
    )
    ...
    if not methods or unknown_methods:
        raise ValueError(
            "Methods must be a nonempty subset of %s" % ",".join(METHODS)
        )
        ...
        for dataset in datasets:
            ...
            for rate in rates:
                for workload in workloads:
                    ...
                    if args.marked_null_group_size:
                        ...
                        print(
                            "Marked-null input: %d missing cells, %d symbols, "
                            "%d repeated symbols" % (
                                marked_statistics["marked_null_count"],
                                marked_statistics["marked_symbol_count"],
                                marked_statistics["marked_repeated_symbol_count"],
                            ),
                            flush=True,
                        )
                    camc_engine = None
                    camc_mapping: Dict[str, Any] = {}
                    sampling_s = 0.0
                    encoding_s = 0.0
                    fallback_symbols = 0
                    if "CAMC" in methods:
                    ...
                    if "CAMC" in methods:
                        print("CAMC setup: factor sampling %.3fs and encoding %.3fs amortized" %
                    ...
                    def checkpoint(partial_rows):
                        ...
                        for partial in partial_rows:
                            ...
                            item["total_factor_sampling_s"] = sampling_s
                            item["total_encoding_s"] = encoding_s
                            item["factor_fallback_symbols"] = fallback_symbols
                            item["null_semantics"] = (
                                "marked" if args.marked_null_group_size
                                else "nonrepeating"
                            )
                            item["marked_null_group_size"] = args.marked_null_group_size
                            item.update(marked_statistics)
                            decorated.append(item)
                        write_csv(args.output, results + decorated)
                    if workload == "set":
                        block_rows = run_set_block(
                            conn, dataset, block, meta, prepared, args.h, args.timeout,
                            sampling_share, encoding_share, camc_engine, camc_mapping,
                            args.query_limit,
                            completed_queries, checkpoint, methods,
                        )
                    else:
                        block_rows = run_agg_block(
                            conn, dataset, block, meta, prepared, factor_map, args.h,
                            args.timeout, sampling_share, encoding_share,
                            camc_engine, camc_mapping,
                            args.query_limit, completed_queries, checkpoint, methods,
                        )
                    for row in block_rows:
                        ...
                        row["marked_null_group_size"] = args.marked_null_group_size
