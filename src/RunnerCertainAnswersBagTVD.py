...
def coverage_aware_tvd(
    pred_counts: Dict[tuple, int],
    gt_counts: Dict[tuple, int],
) -> float:
    """TVD after assigning the smaller result's omitted mass to one deficit outcome."""
    n_pred = sum(pred_counts.values())
    n_gt = sum(gt_counts.values())
    scale = max(n_pred, n_gt)
    if scale == 0:
        return 0.0
    support = set(pred_counts) | set(gt_counts)
    value_distance = sum(
        abs(
            pred_counts.get(value, 0) / scale
            - gt_counts.get(value, 0) / scale
        )
        for value in support
    )
    pred_deficit = (scale - n_pred) / scale
    gt_deficit = (scale - n_gt) / scale
    return 0.5 * (value_distance + abs(pred_deficit - gt_deficit))
...
def evaluate_certain(cur, pred_sql_injected: str, gt_sql_complete: str) -> Dict[str, Any]:
    ...
    # Time the original query under SQL-null semantics separately from the
    # Certain Answers rewriting.
    original_sql = _quote_problem_idents(pred_sql_injected)
    original_sql = _fix_group_by_select(original_sql)
    original_sql = enforce_set_semantics(original_sql)
    t0 = time.perf_counter()
    ...
    bag_support = set(pred_counts) | set(gt_counts)
    if n_pred_bag == 0 and n_gt_bag == 0:
        tv_bag = 0.0
    elif n_pred_bag == 0 or n_gt_bag == 0:
        tv_bag = 1.0
    else:
        tv_bag = 0.5 * sum(
            abs(
                pred_counts.get(value, 0) / n_pred_bag
                - gt_counts.get(value, 0) / n_gt_bag
            )
            for value in bag_support
        )
    tv_coverage_aware = coverage_aware_tvd(pred_counts, gt_counts)
    ...
    return {
        ...
        "time_pred_s": t_pred,
        "time_original_sql_s": t_original,
        "time_ground_truth_s": t_gt,
        "original_sql": original_sql,
        "pred_sql_certain": pred_certain,
        "pred_sql_certain_bag": pred_certain_bag,
        "tv_set": tv_set,
        "tv_bag": tv_bag,
        "coverage_aware_tvd": tv_coverage_aware,
        "js": js,
        "js_sqrt": js_sqrt,
        "coverage": coverage,
